...
class HistoryGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the History entity")
        ...
        .repeat(2) {
            ...
            .post("/api/historys")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "image":"SAMPLE_TEXT", "description":"SAMPLE_TEXT", "calorie":"0", "time":"2020-01-01T00:00:00.000Z"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_history_url")))
            .pause(10)
            .repeat(5) {
